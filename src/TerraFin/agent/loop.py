...
def build_default_system_prompt(definition: TerraFinAgentDefinition) -> str:
    ...
    return (
        ...
        "VIEW CONTEXT: the user is interacting with TerraFin through a UI and almost always has a "
        "specific page open (a stock, a filing, a chart, a screener result). Whenever the user's "
        "message refers to an entity deictically or implicitly — without naming it — assume they "
        "mean what is on their screen and call `current_view_context` BEFORE asking them to "
        "disambiguate. This applies in any language. Asking 'which ticker?' when the user said "
        "something equivalent to 'this stock' is a failure mode: the answer is already in the view. "
        "Only ask for clarification if the tool returns no useful selection. "
        "\n\n"
